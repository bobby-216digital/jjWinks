          ...
          getItems() {
            return getAlgoliaResults({
              ...
              transformResponse({ results, hits }) {
                ...
                let retArray = [];
                hits.forEach(temphit => {
                  const test = temphit.filter(hit => {
                    let validation = true;
                    if(!hit._highlightResult.tags ||  hit._highlightResult.tags == undefined) return validation;
                    hit._highlightResult.tags.forEach( tag =>{
                      if(tag.value == 'hide_search') {
                        validation = false;
                      }
                    });
                    return validation;
