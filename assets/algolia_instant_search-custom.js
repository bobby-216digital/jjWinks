...
(function (algolia) {
  ...
  if(collectionHandle != 'jjsets') {
    facetFilters.push('tags:-hide_search_sets')
  }
    ...
    hits({
      ...
      templates: {
        item(hit, { html, components }) {
          const event = new CustomEvent("timesAct", { detail: { handle: algolia.helpers.instantsearchLink(hit) } });
          setTimeout(() => {
          ...
          if (quickAddSizeButton) {
            ...
            fetch(containerButtonBuy.dataset.productUrl + '.js')
              ...
              .then(async (data) => {
                ...
                const event = new CustomEvent("timesAct", { detail: { handle: data.url } });
                const element = document.querySelector("body");
                element.dispatchEvent(event);  

                const containerCard = containerButtonBuy.closest('.ais-hit');
                if('ta' in window) {
                  const porductPreorder = window.ta.products != null ? window.ta.products[data.id]: null;
                  
                  if (porductPreorder) {
                    if('variants' in porductPreorder) {
                      ...
                      if (variantPreOrder && variantPreOrder.isPOEnabled) {
                        ...
                        containerCard.querySelector('.notification-quick-add').innerHTML = value;
                        containerCard.querySelector('.notification-quick-add').style.display = 'block';
                      } else {
                        if (containerCard.querySelector('.notification-quick-add')) {
                          containerCard.querySelector('.notification-quick-add').innerHTML = '';
                          containerCard.querySelector('.notification-quick-add').style.display = 'none';
                        }
                ...
                if (!selectedVariant || selectedVariant.available == false) {
                  if (containerCard.querySelector('.sold-out-label')) {
                    containerCard.querySelector('.sold-out-label').style.display = 'block';
                  }

                } else {
                  if (containerCard.querySelector('.sold-out-label')) {
                    containerCard.querySelector('.sold-out-label').style.display = 'none';
                  }
                }
              })
              .catch((error) => {
          ...
          if (quickAddButton) {
            ...
            fetch($this.dataset.productUrl + '.js')
              ...
              .then(async (data) => {
                ...
                const containerCard = $this.closest('.ais-hit');
                let value = '';
                if ('ta' in window) {
                  const porductPreorder = window.ta.products != null ? window.ta.products[data.id] : null;
                  if (porductPreorder) {
                ...
                if (!selectedVariant || selectedVariant.available == false) {
                  ...
                  if (value != '') {
                    item.items[0].properties['Pre-order item'] = value;
                    //jsonProduct['properties']['Pre-order item'] = value;
                  }
      ...
      init: function () {
        ...
        container.addEventListener('click', function (e) {
          ...
          if (swatchColor) {
            ...
            const container = $this.closest('.ais-hit--details');
            const targetImage = $this.dataset.targetImage;
            
            const selectedVariantImage = $this.dataset.variantImage;
            const comparePrice = $this.dataset.variantComparePrice ? parseFloat($this.dataset.variantComparePrice).toFixed(2) : null;
            const variantPrice = $this.dataset.variantPrice ? parseFloat($this.dataset.variantPrice).toFixed(2) : null;
            ...
            /* if(container && variantPrice) {
              container.querySelector('.ais-hit--price .price .price-item').innerHTML = `$${variantPrice}`;
              if(comparePrice && parseFloat(variantPrice) < parseFloat(comparePrice)) {
                console.log('comparePrice')
                container.querySelector('.ais-hit--price .price .ais-hit--price-striked').innerHTML = `$${comparePrice}`;
              }
            } */
