...
if (!customElements.get('map-object')) {
  class MapObject extends HTMLElement {
    ...
    async initGoogleMaps() {
      ...
      if (theme.googleMapsApiStatus === 'loaded') {
        ...
        // Load GM api and wait until done
        if (theme.googleMapsApiStatus !== 'loading') {
          ...
          // In case more than one map section might be attempting to load
      ...
      geocoder.geocode({ address: this.addressSetting }, (results, status) => {
        if (status === 'OK') {
          ...
          if (!Shopify.designMode) {
            this.errorMessage(`Geocode was not successful for the following reason - ${status}`);
            return;
          }
